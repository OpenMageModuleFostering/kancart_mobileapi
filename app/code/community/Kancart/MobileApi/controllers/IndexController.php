...
define('MAGENTO_PLUGIN_VERSION', 'v1.2.8');
...
class Kancart_MobileApi_IndexController extends Kancart_MobileApi_Controller_Action {
    ...
    public function indexAction() {
        ...
        try {
            ...
            $this->init();
            $responseContent = $this->processRequestByRequestMethod();
            $this->setResponseBody($responseContent);
        } catch (Exception $e) {
            $result->setReslut('0x0013', null, null, $e->getMessage());
    ...
    public function init() {
        ...
        $this->switchStoreIfNeed($request->getParams());
        $this->switchCurrentCurrencyIfNeed($request->getParams());
    }
    ...
    public function processRequestByRequestMethod() {
        $returnData = '';
        $result = Mage::getModel('mobileapi/Result');
        $request = $this->getRequest();
        switch ($request->getParam('method')) {
            ...
            case 'KanCart.Languages.Get' :
                $returnData = Mage::getModel('mobileapi/Language')->getLanguages($request->getParams());
                break;
            ...
            case 'KanCart.ShoppingCart.Remove' : $returnData = Mage::getModel('mobileapi/Cart')->ShoppingCartRemove($request->getParams());
                break;
            case 'KanCart.ShoppingCart.Get' : $returnData = Mage::getModel('mobileapi/Cart')->ShoppingCartGet($request->getParams());
                break;
    ...
    /**
     * 如果请求参数中的language和当前配置不同则转换当前语言
     * @param type $requestData 
     */
    private function switchStoreIfNeed($requestData) {
        $currentStoreCode = Mage::app()->getStore()->getCode();
        $switchTo = $requestData['language'];
    ...
    /**
     * 如果请求参数中有currency且与当前值不同，则将其设置为当前Currency
     * @param array $request 
     */
    public function switchCurrentCurrencyIfNeed($request) {
        if (isset($request['currency']) && !empty($request['currency'])) {
            if (Mage::app()->getStore()->getCurrentCurrencyCode() != $request['currency']) {
                Mage::app()->getStore()->setCurrentCurrencyCode($request['currency']);
                return true;
            }
        }
        return false;
