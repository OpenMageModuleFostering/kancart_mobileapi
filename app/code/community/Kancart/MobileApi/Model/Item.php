...
class Kancart_MobileApi_Model_Item extends Kancart_MobileApi_Model_Abstract {
    ...
    public function info($productId, $store = null, $attributes = null, $identifierType = null) {
        ...
        if ($product && $product->getId()) {
            ...
            $item['price'] = $this->getViewPrice($item['price']);
...
//            $result[0]['leading_time'] = null;
        ...
        for ($i = 0; $i < count($tiers); $i++) {
            ...
            $result[$i]['price'] = $this->getViewPrice($tiers[$i]['price']);
    ...
    public function getProductGroupedOptions(Mage_Catalog_Model_Product $product) {
        ...
        foreach ($_associatedProducts as $_item) {
            ...
            if ($productPrice > 0.00) {
                ...
                //add
                $sku['price'] = $this->getViewPrice($sku['price']);
            }
            $sku['values'] = array();
            array_push($skus, $sku);
        }
        ...
        if ($product->getTypeInstance(true)->hasOptions($product)) {
            ...
            foreach ($bundleOptions as $_option) {
                ...
                foreach ($selections as $_selection) {
                    ...
                    if ((float) $price != 0.00) {
                        $finalValue['price'] = $this->getViewPrice($price);
                    } else {
     ...
     * @return type 
     */
    private function getConfigurableProductSkusByAttributes($attributes) {
        $skus = array();
        foreach ($attributes as $attrId => $attribute) {
            $eachSku = array();
            $eachSku['sku_id'] = $attrId;
            $eachSku['allow_blank'] = $attribute['is_required'] == '1' ? false : true;
            $eachSku['mode'] = 'select';
            $eachSku['name'] = $attribute['label'];
            $eachSku['values'] = array();
            if (isset($attribute['options'])) {
                foreach ($attribute['options'] as $option) {
                    $value = array();
                    $value['sku_id'] = $attrId;
                    $value['value_id'] = $option['id'];
                    $value['name'] = $option['label'];
                    $value['price'] = $option['price'];
                    $eachSku['values'][] = $value;
                }
            }
            $skus[] = $eachSku;
        }
    ...
    public function getProductOptions(Mage_Catalog_Model_Product $product) {
        ...
        return $this->getConfigurableProductSkusByAttributes($attributes);
    }
    ...
    public function getProductCustomOptionsOption(Mage_Catalog_Model_Product $product) {
        ...
        foreach ($product->getOptions() as $option) {
            ...
            if ($price) {
                $optionObj['price'] = $this->getViewPrice($price);
            } else {
            ...
            if ($type == self::OPTION_TYPE_SELECT) {
                foreach ($option->getValues() as $value) {
                    ...
                    if ($price) {
                        $optionValueObj['price'] = $this->getViewPrice($price);
                    } else {
    ...
    protected function _prepareRecursivelyRelatedValues(&$skuValue, $attributes, $productIds, $cycle = 1) {
        ...
        foreach ($attribute['options'] as $option) {
            ...
            if ((float) $option['price'] != 0.00) {
                $finalValue['price'] = $this->getViewPrice($option['price']);
            } else {
                $finalValue['price'] = null;
            }
