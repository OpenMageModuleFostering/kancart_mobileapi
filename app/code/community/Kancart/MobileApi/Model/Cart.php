...
class Kancart_MobileApi_Model_Cart extends Kancart_MobileApi_Model_Abstract {
    ...
    /**
     * Cart Item Render 的工厂方法
     * @param type $product 
     */
     public function createCartItemRender($item) {
        switch ($item->getProductType()) {
            case 'configurable':
                return new Mage_Checkout_Block_Cart_Item_Renderer_Configurable();
            case 'bundle':
                return new Mage_Bundle_Block_Checkout_Cart_Item_Renderer();
            default:
                return new Mage_Checkout_Block_Cart_Item_Renderer();
                break;
        }
    }
     ...
     * @return type
     */
    public function getCartDisplaySkus($render) {
        $optionList = $render->getOptionList();
        return $optionList;
    }
